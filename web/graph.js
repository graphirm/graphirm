...
export function initGraph() {
  ...
  document.querySelector('[data-action="subgraph"]').addEventListener('click', async () => {
    const detail = document.getElementById('node-detail');
    if (detail.dataset.nodeId) {
      try {
        const subgraph = await api.getSubgraph(getCurrentSessionId(), detail.dataset.nodeId);
        _currentData = subgraph;
...
export function renderGraphData(data) {
  _currentData = data || { nodes: [], edges: [] };
  renderGraph(_currentData);
}
...
function renderGraph({ nodes, edges }) {
  ...
  const link = g.append('g').selectAll('line')
    ...
    .join('line')
    .attr('stroke', d => {
      const key = normalizeEdgeType(d.edge_type) ?? d.edge_type;
      const color = EDGE_COLORS[key];
      return color ?? '#ffffff22';
    })
    .attr('stroke-width', d => {
      const key = normalizeEdgeType(d.edge_type) ?? d.edge_type;
      return EDGE_STROKE_WIDTH[key] ?? EDGE_STROKE_WIDTH.default;
    })
    .attr('opacity', 0.7)
  ...
  const node = g.append('g').selectAll('circle')
    ...
    .on('click', async (_, d) => {
      try {
        const node = await api.getNode(getCurrentSessionId(), d.id);
        showNodeDetail(node);
      } catch (err) {
