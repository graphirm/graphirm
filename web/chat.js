/**
 * Chat module for Graphirm browser UI.
 * Adapted from graphirm-vscode/media/chat.js — uses api.js instead of _send callback.
 */
...
export function initChat() {
  ...
  abortBtn.addEventListener('click', () => {
    ...
    if (id) api.abortSession(id).catch((err) => console.error('Abort failed:', err));
  });
}
...
async function sendPrompt() {
  ...
  if (!id) {
    ...
    try {
      const session = await api.createSession(name.trim() || `session-${Date.now()}`);
      const { onSessionCreatedFromChat } = await import('./main.js');
      onSessionCreatedFromChat(session);
  ...
  api.sendPrompt(id, content).catch((err) => console.error('Send failed:', err));
  input.value = '';
...
export function flushPendingPrompt(sessionId) {
  ...
  _pendingPrompt = null;
  api.sendPrompt(sessionId, content).catch((err) => console.error('Flush failed:', err));
  appendMessage({ role: 'user', content, id: `optimistic-${Date.now()}` });
}
...
function toggleHitlPause(sessionId, btn) {
  ...
  fetch(`/api/sessions/${sessionId}/${endpoint}`, { method: 'POST' }).catch((err) =>
    console.error('[HITL] pause/resume failed:', err)
  );
...
window.hitlResume = function hitlResume(sessionId, nodeId) {
  fetch(`/api/sessions/${sessionId}/resume`, { method: 'POST' }).catch((err) =>
    console.error('[HITL] resume request failed:', err)
  );
