...
pub fn tasks() -> Vec<EvalTask> {
    vec![
        EvalTask {
            ...
            prompts: vec![
                "Rust uses ownership and the borrow checker to guarantee memory safety \
                 at compile time without a garbage collector. The borrow checker is a \
                 static analysis pass in rustc."
                    .to_string(),
            ],
        ...
        EvalTask {
            ...
            prompts: vec![
                "The system uses three key patterns: \
                 1) PageRank for relevance scoring of graph nodes, \
