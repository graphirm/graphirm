...
impl TestHarness {
    ...
    async fn run_task_inner(&self, task: &EvalTask) -> anyhow::Result<TaskResult> {
        let session = self
            .client
            .create_session(task.enable_segments, task.segment_filter.as_deref())
            .await?;
        ...
        let prompts_result = self
            .run_task_prompts(task, &session_id, &mut last_response, &mut turns_used)
            .await;
        ...
        let mut r = match prompts_result {
            ...
            Ok(_) => {
                match self
                    .check_verifier(&task.verifier, &session_id, &last_response)
                    .await
                {
    ...
    async fn run_task_prompts(
        ...
        for prompt in &task.prompts {
            ...
            let status = self
                .client
                .wait_for_idle(session_id, task.timeout_secs)
                .await?;
