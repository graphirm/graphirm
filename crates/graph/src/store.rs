...
impl GraphStore {
    ...
    /// Fetch all Interaction nodes spawned by an agent/session, ordered by creation.
    ///
    /// Returns all Interaction nodes that have a "spawned_by" edge pointing to the given session_id.
    ...
    /// Fetch all tool result nodes that respond to a given node.
    ///
    /// Returns all nodes that have a "responds_to" edge pointing to the given node_id.
