...
echo ""
echo "S1 Metrics:"
curl -s "http://localhost:5555/api/sessions/$S1" | jq '.metrics' | tee s1_metrics.json
...
echo ""
echo "S2 Metrics:"
curl -s "http://localhost:5555/api/sessions/$S2" | jq '.metrics' | tee s2_metrics.json
...
echo ""
echo "S3 Metrics:"
curl -s "http://localhost:5555/api/sessions/$S3" | jq '.metrics' | tee s3_metrics.json
