...
Coverage: **10%** (up from ~2%). Done so far: `EmbeddingCache` (Python fallback, 76%), `InsightsLoader` (100%), `run_full_graph_pipeline` (82%) — 90 tests passing. Fixed import-time blockers: lazy `scipy` guard in `spectral.py` (remove `sp.csr_matrix` annotations, `_SCIPY_AVAILABLE` flag) and lazy `mcp` server in `mcp/__init__.py` (commit `38782ce` on ndstrms). Remaining targets: `FileTreeWalker`, `TFIDFOrchestrator`, `MinHashLSH`, `SymbolExtractor`, `CallExtractor`, `Layer4CondensationEngine`, `RepoInsightExtractor`.
