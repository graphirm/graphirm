...
- **Planning ↔ files:** `graph_query` `project` **`link_session`** links the session Agent to a planning Knowledge node (`DerivedFrom`). With **`auto_link_write_to_planning = true`** (default in `AgentConfig`), **`write`** / **`edit`** on **`file`** Content nodes add **`relates_to`** (planning → file, metadata **`artifact_link`**: `implements`) when that link exists; set **`auto_link_write_to_planning = false`** in **`[agent]`** to disable. Manual **`link_content`** remains available.
...
| 53 | Automated trace analysis — `trace_analysis.rs`: `SessionDigest` extractor, 5 pattern detectors (over_tooling, doom_loops, token_waste, tool_errors_without_recovery, premature_completion), `build_trace_report` aggregation + suggestions; `graphirm trace-analysis` CLI; `GET /api/trace-analysis` endpoint; 23 new tests | ✅ done |
